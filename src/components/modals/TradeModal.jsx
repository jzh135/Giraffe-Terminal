    ...
    if (editingTransaction) {
      ...
      const type =
        editingTransaction.type === 'transaction'
          ? editingTransaction.tx_type
          : editingTransaction.type; // Normalized type
          ...
          >
            <div className="modal-header">
              <h2 className="modal-title">
                Confirm {getTabLabel(pendingTab)}
            ...
            <p style={{ color: 'var(--text-secondary)', marginBottom: 'var(--spacing-lg)' }}>
              ...
              {pendingRows.length === 1 ? 'entry' : `${pendingRows.length} entries`} before
              confirming:
            </p>
                  ...
                  >
                    <div>
                      ...
                      <div style={{ fontWeight: 500 }}>
                        <span
                          className={`badge ${pendingTab === 'buy' ? 'badge-success' : pendingTab === 'sell' ? 'badge-danger' : 'badge-warning'}`}
                        >
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <div style={{ fontWeight: 600 }}>
                            {formatCurrency(parseFloat(row.shares) * parseFloat(row.price))}
                          </div>
                        </div>
                      </>
                    )}
                          ...
                          <>
                            ...
                            <div>
                              ...
                              <div
                                style={{
                                  fontWeight: 600,
                                  color: realizedGain >= 0 ? 'var(--success)' : 'var(--danger)',
                                }}
                              >
                                {realizedGain >= 0 ? '+' : ''}
    ...
    <>
      <div className="modal-overlay" onClick={onClose}>
        <div
          className="modal"
          onClick={(e) => e.stopPropagation()}
          style={{ maxWidth: activeTab === 'sell' ? '1100px' : '1000px', width: '95%' }}
        >
          <div className="modal-header">
          ...
          <form onSubmit={handleReview}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table
                ...
                style={{ minWidth: activeTab === 'sell' ? '900px' : '800px' }}
              >
                <thead>
                  <tr>
                    {activeTab === 'buy' && (
                            ...
                            <input
                              ...
                              value={
                                row.shares && row.price
                                  ? (parseFloat(row.shares) * parseFloat(row.price)).toFixed(2)
                                  : '-'
                              }
                              readOnly
                              tabIndex="-1"
                                ...
                                >
                                  ...
                                  {getAvailableLots(row.symbol).map((lot) => (
                                    <option key={lot.id} value={lot.id}>
                                      ...
                                      • {lot.shares} shares @ $
                                      {(lot.cost_basis / lot.shares).toFixed(2)}
                                    </option>
                                  ))}
                                </select>
                              </div>
