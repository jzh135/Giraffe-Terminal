...
const ETF_SYMBOLS = ['SPY', 'VOO', 'VTI', 'QQQ', 'IWM', 'VEA', 'VWO', 'BND', 'AGG', 'VNQ', 'GLD', 'SLV',
    'XLF', 'XLE', 'XLK', 'XLV', 'XLP', 'XLY', 'XLI', 'XLB', 'XLU', 'XLRE', 'VGT', 'VHT', 'VIG', 'VYM',
    'ARKK', 'ARKG', 'ARKF', 'ARKW', 'ARKQ', 'SCHD', 'JEPI', 'JEPQ', 'SPHD', 'HDV', 'DVY'];
...
export default function Analysis() {
    ...
    async function loadData() {
        try {
            ...
            const [holdingsData, pricesData] = await Promise.all([
                api.getHoldings(),
                api.getPrices()
            ]);
    ...
    function calculateAnalytics(holdingsData, priceLookup) {
        ...
        Object.values(symbolTotals).forEach(stock => {
            ...
            const hasTargets = priceData.target_median_price > 0 ||
            ...
            const medianGrowth = medianTarget > 0 && currentPrice > 0
                ? ((medianTarget - currentPrice) / currentPrice) * 100
                : null;
            const buyDistance = buyTarget > 0 && currentPrice > 0
                ? ((currentPrice - buyTarget) / buyTarget) * 100
                : null;
            const sellDistance = sellTarget > 0 && currentPrice > 0
        ...
        const weightedMedianGrowthStocksOnly = totalValueStocksOnly > 0 ? weightedGrowthSumStocksOnly / totalValueStocksOnly : 0;
    ...
    return (
        <div className="page-container">
            ...
            <div className="card mb-lg" style={{
                ...
                <div className="stats-grid">
                    <div>
                        ...
                        <div className={`stat-value ${analytics.weightedMedianGrowth >= 0 ? 'text-positive' : 'text-negative'}`}>
                            {formatPercent(analytics.weightedMedianGrowth)}
                        </div>
                    </div>
                    <div>
                        <div className="text-muted">Weighted Growth (Stocks Only)</div>
                        <div className={`stat-value ${analytics.weightedMedianGrowthStocksOnly >= 0 ? 'text-positive' : 'text-negative'}`}>
                    ...
                    <div>
                        ...
                        <div className="stat-value" style={{ color: '#10b981' }}>{analytics.nearBuy.length}</div>
                    </div>
                    <div>
                        <div className="text-muted">Near Sell Zone</div>
                        <div className="stat-value" style={{ color: '#ef4444' }}>{analytics.nearSell.length}</div>
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                {analytics.nearBuy.map(stock => (
                                    ...
                                    >
                                        ...
                                        <td style={{ textAlign: 'right', color: '#10b981' }}>{formatCurrency(stock.buyTarget)}</td>
                                        <td style={{ textAlign: 'right' }}>
                                            <span className={stock.buyDistance <= 0 ? 'text-positive' : 'text-muted'}>
                                                {stock.buyDistance <= 0 ? '✓ Below target' : `${stock.buyDistance.toFixed(1)}% above`}
                                            </span>
                                        </td>
                                    </tr>
                                ))}
            ...
            <div className="card mb-lg">
                <h3 style={{ margin: 0, marginBottom: '16px', color: '#ef4444' }}>
                    🔴 Near Sell Zone
                </h3>
                <p className="text-muted" style={{ marginBottom: '16px', fontSize: '0.85rem' }}>
                            ...
                            <tbody>
                                {analytics.nearSell.map(stock => (
                                    ...
                                    >
                                        ...
                                        <td>{stock.name}</td>
                                        <td style={{ textAlign: 'right' }}>{formatCurrency(stock.currentPrice)}</td>
                                        <td style={{ textAlign: 'right', color: '#ef4444' }}>{formatCurrency(stock.sellTarget)}</td>
                                        <td style={{ textAlign: 'right' }}>
                                            <span className={stock.sellDistance <= 0 ? 'text-negative' : 'text-muted'}>
                                                {stock.sellDistance <= 0 ? '✓ Above target' : `${stock.sellDistance.toFixed(1)}% below`}
                                            </span>
                                        </td>
                                    </tr>
                                ))}
            ...
            <div className="card">
                ...
                {analytics.allTargets.length === 0 ? (
                    <p className="text-muted">No stocks have price targets set. Add targets in the Research page for each stock.</p>
                ) : (
                    <div className="table-container">
                        <table className="data-table">
                            <thead>
                                ...
                                {sortedTargets.map(stock => (
                                    ...
                                    >
                                        <td>
                                            ...
                                            {stock.isETF && <span className="text-muted" style={{ marginLeft: '4px', fontSize: '0.75rem' }}>(ETF)</span>}
                                        </td>
                                        <td>{stock.name}</td>
                                        <td style={{ textAlign: 'right' }}>{formatCurrency(stock.currentPrice)}</td>
                                        <td style={{ textAlign: 'right' }}>
                                        ...
                                        <td style={{ textAlign: 'right' }}>
                                            {stock.medianGrowth !== null ? (
                                                <span className={stock.medianGrowth >= 0 ? 'text-positive' : 'text-negative'}>
