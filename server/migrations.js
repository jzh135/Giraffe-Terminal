...
export function runMigrations(db, __dirname) {
  ...
  try {
    ...
    if (hasRoleText && hasRoleId) {
      const stocks = db
        .prepare('SELECT symbol, role FROM stock_prices WHERE role IS NOT NULL AND role_id IS NULL')
        .all();
      ...
      for (const stock of stocks) {
        ...
        if (roleRecord) {
          db.prepare('UPDATE stock_prices SET role_id = ? WHERE symbol = ?').run(
            roleRecord.id,
            stock.symbol
          );
        }
    ...
    if (hasThemeText) {
      const stocks = db
        .prepare(
          'SELECT symbol, theme FROM stock_prices WHERE theme IS NOT NULL AND theme_id IS NULL'
        )
        .all();
      if (stocks.length > 0) console.log('Migrating data: Converting theme text to IDs...');
      for (const stock of stocks) {
        const themeRecord = db
          .prepare('SELECT id FROM stock_themes WHERE name = ?')
          .get(stock.theme);
        if (themeRecord) {
          db.prepare('UPDATE stock_prices SET theme_id = ? WHERE symbol = ?').run(
            themeRecord.id,
            stock.symbol
          );
        }
