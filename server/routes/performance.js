...
router.get('/', async (req, res) => {
  try {
    ...
    const twr = calculateModifiedDietzWithStartValue(
      cashFlows,
      startValue,
      portfolioValue,
      ytdStartDate
    ...
    holdings.forEach((h, index) => {
      ...
      if (!allocation[groupName]) {
        allocation[groupName] = {
          ...
          color:
            group_by === 'stock'
              ? stockColors[Object.keys(allocation).length % stockColors.length]
              : groupColor,
        };
      }
...
router.get('/chart', async (req, res) => {
  try {
    ...
    const fetchStartDate =
      allTimeStartDate < oneYearStartDate ? allTimeStartDate : oneYearStartDate;
    ...
    const accountData = await Promise.all(
      accounts.map(async (account, index) => {
        const history = await getAccountPerformanceHistoryWithPrices(
          account.id,
          startDate,
          endDate,
          historicalPrices
...
router.get('/recalculate/:accountId', async (req, res) => {
  ...
  try {
    ...
    const deleteResult = db
      .prepare('DELETE FROM performance_history WHERE account_id = ?')
      .run(accountId);
    console.log(
      `Deleted ${deleteResult.changes} old performance history records for account ${accountId}`
    );
      ...
      for (const h of holdings) {
        ...
        if (price === null) {
          const currentPrice = db
            .prepare('SELECT price FROM stock_prices WHERE symbol = ?')
            .get(h.symbol);
    ...
    console.log(
      `Regenerated ${regenerated} performance history records for account ${accountId} in ${totalElapsed}s`
    );
...
router.get('/history', async (req, res) => {
  try {
    ...
    activities = activities.concat(
      cashMovements.map((c) => ({ date: c.date, cashChange: c.amount }))
    );
...
function getDateRange(timeframe, accounts) {
  ...
  if (timeframe === 'YTD') {
    ...
    const earliestCashMove = db.prepare('SELECT MIN(date) as earliest FROM cash_movements').get();

    startDate =
      earliestCashMove?.earliest && earliestCashMove.earliest < earliestAccount
...
async function getSPYHistory(startDate, endDate) {
  ...
  const today = new Date().toISOString().split('T')[0];
  const hasRecentData = cached.some(
    (c) => c.date >= today || (new Date(today) - new Date(c.date)) / (1000 * 60 * 60 * 24) <= 1
  );
...
async function getAccountPerformanceHistoryWithPrices(
  accountId,
  startDate,
  endDate,
  historicalPrices
