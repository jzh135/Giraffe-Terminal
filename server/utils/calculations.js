...
export function calculateCashBalance(db, accountId) {
  ...
  const cashMovements = db
    .prepare(`SELECT COALESCE(SUM(amount), 0) as total FROM cash_movements ${whereClause}`)
    .get(...params);

  // Dividends received
  const dividends = db
    .prepare(`SELECT COALESCE(SUM(amount), 0) as total FROM dividends ${whereClause}`)
    .get(...params);
  ...
  return (
    (cashMovements?.total || 0) + (dividends?.total || 0) - (buys?.total || 0) + (sells?.total || 0)
  );
}
...
export function calculatePortfolioValue(db, accountId) {
  ...
  const holdings = db
    .prepare(`SELECT symbol, SUM(shares) as shares FROM holdings ${whereClause} GROUP BY symbol`)
    .all(...params);
...
export function calculateRealizedGain(db, accountId) {
  ...
  const dividends = db
    .prepare('SELECT COALESCE(SUM(amount), 0) as total FROM dividends WHERE account_id = ?')
    .get(accountId);
