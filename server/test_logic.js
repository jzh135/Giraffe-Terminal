...
function runTest() {
  ...
  const accountResult = db
    ...
    .run('Test Account ' + Date.now(), 'brokerage');
  ...
  try {
    ...
    db.prepare(
      'INSERT INTO cash_movements (account_id, type, amount, date) VALUES (?, ?, ?, ?)'
    ).run(accountId, 'deposit', depositAmount, '2024-01-01');
    ...
    const holdingRes = db
      .prepare(
        'INSERT INTO holdings (account_id, symbol, shares, cost_basis, purchase_date) VALUES (?, ?, ?, ?, ?)'
      )
      .run(accountId, symbol, shares, costBasis, '2024-01-02');
    const holdingId = holdingRes.lastInsertRowid;

    // Create Transaction
    db.prepare(
      'INSERT INTO transactions (account_id, holding_id, type, symbol, shares, price, total, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(accountId, holdingId, 'buy', symbol, shares, price, costBasis, '2024-01-02');
    console.log(`Bought ${shares} shares of ${symbol} at ${price}`);

    // Define Price for Portfolio Value
    // Ensure stock_prices has entry
    db.prepare(
      "INSERT OR REPLACE INTO stock_prices (symbol, price, updated_at) VALUES (?, ?, datetime('now'))"
    ).run(symbol, price); // Price is same as buy
    ...
    if (portfolioValue !== expectedValue)
      throw new Error('Portfolio Value mismatch after fluctuation');
    ...
    // Transaction
    db.prepare(
      'INSERT INTO transactions (account_id, holding_id, type, symbol, shares, price, total, date, realized_gain) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(
      accountId,
      holdingId,
      'sell',
